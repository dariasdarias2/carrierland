...
if ~isequal(Qahat-Qahat'<1E-6,ones(size(Qahat)));
  warning ('Variance-covariance matrix is not symmetric!');
  afixed = [ahat, ahat];
  sqnorm = [1, 1];
  return;
end;
...
if length(ahat) ~= size(Qahat,1);
  warning (['Variance-covariance matrix and vector of ambiguities do', ...
...
switch method
    ...
    case 5       
        ...
        %%%%% Adding the FFRT to PAR
        if 1-Ps > P0
            mu = ratioinv(P0,1-Ps,nfixed);
